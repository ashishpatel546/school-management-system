...
export default function BulkPromotionsPage() {
                ...
                const filtered = allStudents.filter((s: any) => {
                    ...
                    if (s.enrollments) {
                        return s.enrollments.some((e: any) =>
                            ...
                            (!fromSectionId || e.section?.id === parseInt(fromSectionId)) &&
                            e.status === 'ACTIVE'
                        );
                    }
                    // Fallback to legacy check if enrollments aren't fully populated yet
                    return s.class?.id === parseInt(fromClassId) && (!fromSectionId || s.section?.id === parseInt(fromSectionId));
                });
    ...
    const handleAction = async () => {
        ...
        if (actionType === "PROMOTE") {
            ...
            try {
                setLoading(true);
                const res = await fetch(`${API_BASE_URL}/students/exits/bulk`, {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify(payload)
                });
