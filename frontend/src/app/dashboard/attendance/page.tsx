...
export default function AttendancePage() {
    ...
    useEffect(() => {
        const fetchSession = async () => {
            ...
            try {
                const res = await fetch(`${API_BASE_URL}/academic-sessions/by-date?date=${selectedDate}`);
                if (res.ok) {
    ...
    useEffect(() => {
        const fetchStudents = async () => {
            if (selectedClassId && selectedSectionId && activeSession) {
                ...
                try {
                    const res = await fetch(`${API_BASE_URL}/students?classId=${selectedClassId}&sectionId=${selectedSectionId}&academicSessionId=${activeSession.id}`);
                    if (res.ok) {
    ...
    const getChartData = () => {
        ...
        const stats = { PRESENT: 0, ABSENT: 0, LATE: 0, HALF_DAY: 0 };
                                    ...
                                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                                        ...
                                        <div className="bg-yellow-50 p-4 rounded-lg border border-yellow-100 text-center">
                                            ...
                                            <p className="text-2xl font-black text-yellow-900">
                                                {Object.values(attendanceRecords).filter(r => r.status === 'LATE' || r.status === 'HALF_DAY').length}
                                            </p>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    {getAbsentStudents().length === 0 ? (
                                        ...
                                        getAbsentStudents().map(student => (
                                            <div key={student.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-100 hover:bg-gray-100 transition-colors">
                                                ...
                                                <span className={`text-[10px] font-bold px-2 py-1 rounded-sm uppercase tracking-wider ${attendanceRecords[student.id]?.status === 'ABSENT' ? 'bg-red-100 text-red-700' : 'bg-purple-100 text-purple-700'}`}>
                                                    {attendanceRecords[student.id]?.status.replace('_', ' ')}
                                                </span>
                                            </div>
                                        ))
                                    )}
